...
| `bin/console topdata:mapper:count` | Count shop products with EAN/MPN (pretty table, DB-only debug helper; article numbers skipped — mandatory in SW6; placeholders `-`/`n/a` excluded by default, `--show-placeholders` adds them as sub-rows; `--also-customfields` per custom field, `--parents-only` excludes variants) |
...
- **`tdmp_brand` schema contract** — columns are `product_manufacturer_id`, `product_manufacturer_version_id`, `topdata_brand_id` (Topdata brand id), `created_at`, `updated_at`. `product_manufacturer_version_id` is **always the live version** (`TdmpBrandService::LIVE_VERSION_HEX`); `insertMany()` pins it. The FK `fk_tdmp_brand_product_manufacturer (product_manufacturer_id, product_manufacturer_version_id) → product_manufacturer(id, version_id) ON DELETE CASCADE` is composite because manufacturers are versioned (same draft-merge rationale as the product FK). The table is **only read by this plugin** (DSL matcher's `topdataBrandIds` leaf via the reverse brand→product map); TopFeed / TopFinder never touch it.
...
- **Admin**: two modules — settings (`topdata.mapper.settings`, strategy editor: preset chips, DSL textarea with debounced live validation and a syntax help modal) and conflicts (`topdata.mapper.conflicts`, under Products nav, immediate radio resolve without re-import). Backend: `TopdataMapperActionController` (`_routeScope: api`), privileges `topdata_mapper:read` / `topdata_mapper:update` (`Resources/config/acl.xml`). The DSL parser and serializer both live once in PHP (`src/Service/Dsl/`) — the settings page only sends/validates DSL strings, so there is no JS mirror to drift. The grammar supports `( )` groups for explicit precedence (`&` binds tighter than `|`).
