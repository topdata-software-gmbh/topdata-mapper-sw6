...
- **`tdmp_product` schema contract** — columns are `product_id`, `product_version_id`, `top_id` (Topdata products_id), `created_at`, `updated_at`. `product_version_id` is **always the live version** (`TdmpProductService::LIVE_VERSION_HEX`, mirrors `Shopware\Core\Defaults::LIVE_VERSION`); `insertMany()` pins it. The FK `fk_tdmp_product_product (product_id, product_version_id) → product(id, version_id) ON DELETE CASCADE` is composite because products are versioned — draft rows deleted during version merges never match the FK, only real product deletions cascade. Matchers MUST return live-version products only. Consumers (TopFeed, TopFinder) read `top_id` — keep them in sync.
...
- **Pluggable matcher**: `TdmpMappingBuildService` depends on `ProductMappingMatcherInterface`; the default `ProductMappingMatcher_EanMpn` (ean↔`product.ean`, oem↔`product.manufacturer_number`) is wired in `services.xml`. Keep the interface stable — TopFeed supplies its own matcher (`ProductMappingMatcher_TopFeed`). `matchRow()` returns `list<array{product_id: string}>` (hex, live version only).
