...
- **`topdata:mapper:count` debug command** — counts live Shopware products
  with EAN / MPN values (plus totals) and, with `--also-customfields`, one
  row per custom-field name; `--parents-only` excludes variants. DB-side
  only, no API call. Article numbers are not counted (mandatory in Shopware
  6, always 100%). Counts exclude junk placeholder values (`-`, `n/a`,
  no-digit EANs) by default; `--show-placeholders` adds the excluded
